...
class AddItemToCartUseCaseTest {
    ...
    void execute_AddNewItem_Success() {
        ...
        when(inventoryChecker.getAvailableStock(productId)).thenReturn(10);
        when(cartRepository.save(any(Cart.class))).thenReturn(cart);
        
        AddItemToCartCommand command = new AddItemToCartCommand(cartId, productId, 3, options);
        ...
        verify(inventoryChecker).getAvailableStock(productId);
    ...
    void execute_AddExistingItemWithDifferentOptions_CreateSeparateItem() {
        ...
        AddItemToCartCommand command = new AddItemToCartCommand(cartId, productId, 1, differentOptions);
    ...
    void execute_InsufficientStock() {
        // given
        cart.addItem(productId, 5, options);
        
        when(cartRepository.findById(cartId)).thenReturn(Optional.of(cart));
        when(inventoryChecker.getAvailableStock(productId)).thenReturn(7);
    ...
    void execute_AlreadyConverted() {
        // given
        cart.addItem(ProductId.of("PROD-999"), 1, ProductOptions.empty());
        cart.convertToOrder();
    ...
    void execute_ExceedItemLimit() {
        ...
        when(cartRepository.findById(cartId)).thenReturn(Optional.of(cart));
        when(inventoryChecker.getAvailableStock(newProductId)).thenReturn(10);
        
        AddItemToCartCommand command = new AddItemToCartCommand(cartId, newProductId, 1, ProductOptions.empty());
        ...
        verify(cartRepository).findById(cartId);
        verify(inventoryChecker).getAvailableStock(newProductId);
        verify(cartRepository, never()).save(any());
    }
    ...
    void execute_InvalidQuantity() {
        ...
        // when & then - 수량 0
        assertThatThrownBy(() -> new AddItemToCartCommand(cartId, productId, 0, options))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessageContaining("Quantity must be between 1 and 99");
        
        // when & then - 수량 100
        assertThatThrownBy(() -> new AddItemToCartCommand(cartId, productId, 100, options))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessageContaining("Quantity must be between 1 and 99");
    }
    ...
    void execute_ExactStock_Success() {
        // given
        when(cartRepository.findById(cartId)).thenReturn(Optional.of(cart));
        when(inventoryChecker.getAvailableStock(productId)).thenReturn(5);
        when(cartRepository.save(any(Cart.class))).thenReturn(cart);
        
        AddItemToCartCommand command = new AddItemToCartCommand(cartId, productId, 5, options);
